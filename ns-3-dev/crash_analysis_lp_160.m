...
clear all;
close all;
%filename = '/Users/avikdayal/Google-Drive/PHD-Thesis/ARL-work/ns-3-beacon-rate/ns-3-dev/bsm_80node_cvgauss_lp.output3.csv_msgStats3.csv';
%filename = '/Users/avikdayal/Google-Drive/PHD-Thesis/ARL-work/ns-3-beacon-rate/ns-3-dev/bsm_80node_cvgauss.output3.csv_msgStats3.csv';
%filename = '/Users/avikdayal/Google-Drive/PHD-Thesis/ARL-work/ns-3-beacon-rate/ns-3-dev/bsm_40node_cvgauss.output3.csv_msgStats3.csv';
...
for i=1:length(sit)
    ...
    temp_matrix=(D(:,2)==sit(i,1) & D(:,3)==sit(i,2));
    temp_index=find(temp_matrix==1);
    ttc_array=D(temp_index,4)';
    recv_array=D(temp_index,6)';
    toc
...
%     end
    ...
    %find((ttc_array-ttc_array_2)==0);
    %display('finished array search')
    %tic
    ...
    if(length(ind)~=0)
        ...
        for k=1:length(p)-1
           ind(p(k)):ind(p(k+1));
           ind_recv=find(recv_array(ind(p(k)):ind(p(k+1)))==0);
           if(length(ind_recv)==length(ind(p(k)):ind(p(k+1))))
               crash=crash+1
           end
        end
    end
