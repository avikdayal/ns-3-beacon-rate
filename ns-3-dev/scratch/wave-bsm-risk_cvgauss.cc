 ...
 */
 namespace ns3{
 void printStats(std::string CSVfileName3);
};
class RoutingStats
{
public:
...
RoutingHelper::SetupRoutingPacketReceive (Ipv4Address addr, Ptr<Node> node)
{
  ...
  sink->Bind (local);
  std::cout << "/* about to setup routing packet */" << '\n';
...
RoutingHelper::ReceiveRoutingPacket (Ptr<Socket> socket)
{
  Ptr<Packet> packet;
  Address srcAddress;
  //std::cout << "/* received routing packet info  */" << std::endl;
  while ((packet = socket->RecvFrom (srcAddress)))
    {
      ...
      //std::cout << "/* received routing packet info  */" << PrintReceivedRoutingPacket (socket, packet, srcAddress) << std::endl;
      if (m_log != 0)
        {
...
std::map <uint32_t, Vector> velocities;
/**
 * \ingroup wave
...
private:
  ...
  std::string m_CSVfileName3; ///< CSV file name
...
VanetRoutingExperiment::VanetRoutingExperiment ()
  : m_port (9),
    ...
    m_CSVfileName3 ("vanet-routing.output3.csv"),
    m_nSinks (10),
...
static ns3::GlobalValue g_CSVfileName2 ("VRCCSVfileName2",
                                        ...
                                        ns3::MakeStringChecker ());
static ns3::GlobalValue g_CSVfileName3 ("VRCCSVfileName3",
                                        "CSV filename 3 (for overall simulation scenario results)",
                                        ns3::StringValue ("vanet-routing.output3.csv"),
                                        ns3::MakeStringChecker ());
...
VanetRoutingExperiment::ConfigureDevices ()
{
  ...
  //Config::Connect("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxBegin", MakeCallback(&PhyTxCallbackTrace));
...
VanetRoutingExperiment::ProcessOutputs ()
{
  ...
  double bsm_pdr1_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (1);
  double bsm_pdr2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (2);
  double bsm_pdr3_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (3);
  double bsm_pdr4_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (4);
  double bsm_pdr5_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (5);
  double bsm_pdr6_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (6);
  double bsm_pdr7_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (7);
  double bsm_pdr8_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetCumulativeBsmPdr (8);
  ...
  uint32_t cumulativeWaveBsmBytes_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetTxByteCount ();
...
{
  std::cout << "beginning of check throughput" << '\n';
  ...
  int wavePktsSent_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetTxPktCount ();
  ...
  int waveExpectedRxPktCount_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetExpectedRxPktCount (1);
  int waveRxPktInRangeCount_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetRxPktInRangeCount (1);
  ...
  double wavePDR1_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (1);
  double wavePDR2_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (2);
  double wavePDR3_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (3);
  double wavePDR4_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (4);
  double wavePDR5_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (5);
  double wavePDR6_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (6);
  double wavePDR7_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (7);
  double wavePDR8_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (8);
  double wavePDR9_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (9);
  double wavePDR10_2_hp = m_waveBsmHelper.GetWaveBsmStats_hp ()->GetBsmPdr (10);
  ...
  out << (Simulator::Now ()).GetSeconds () << ","
      ...
      << waveExpectedRxPktCount_hp << ","
      << waveRxPktInRangeCount << ","
      << waveRxPktInRangeCount_hp << ","
      << wavePDR1_2 << ","
      << wavePDR2_2 << ","
      << wavePDR3_2 << ","
      << wavePDR4_2 << ","
  ...
  m_waveBsmHelper.GetWaveBsmStats_hp ()->SetRxPktCount (0);
  m_waveBsmHelper.GetWaveBsmStats_hp ()->SetTxPktCount (0);
  for (int index = 1; index <= 10; index++)
    {
      ...
      m_waveBsmHelper.GetWaveBsmStats_hp ()->SetExpectedRxPktCount (index, 0);
      m_waveBsmHelper.GetWaveBsmStats_hp ()->SetRxPktInRangeCount (index, 0);
    }
...
VanetRoutingExperiment::SetConfigFromGlobals ()
{
  ...
  GlobalValue::GetValueByName ("VRCCSVfileName3", stringValue);
  m_CSVfileName3 = stringValue.Get ();
...
VanetRoutingExperiment::SetGlobalsFromConfig ()
{
  ...
  g_CSVfileName3.SetValue (StringValue (m_CSVfileName3));
...
VanetRoutingExperiment::CommandSetup (int argc, char **argv)
{
  ...
  cmd.AddValue ("CSVfileName3", "The name of the CSV output file name3", m_CSVfileName3);
  ...
  cmd.Parse (argc, argv);
  std::cout << "/* protocol after parse */" << m_CSVfileName3 << '\n';
  m_txSafetyRange1 = txDist1;
  m_txSafetyRange2 = txDist2;
  m_txSafetyRange3 = txDist3;
  m_txSafetyRange4 = txDist4;
...
VanetRoutingExperiment::SetupAdhocMobilityNodes ()
{
  if (m_mobility == 1)
  ...
  else if (m_mobility == 2)
    {
      ...
      pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=1000.0]")); //Distance to 1 km
...
{
  std::cout << "loss model" << m_lossModel << '\n';
  if (m_lossModel == 1)
    {
  ...
  if (m_fading != 0)
    {
      ...
      std::cout << "nakagami propagation loss" << '\n';
    }
  ...
  // Setup net devices
  std::cout << "/* 80211 mode */" << m_80211mode << '\n';
  if (m_80211mode == 3)
    {
...
VanetRoutingExperiment::SetupWaveMessages ()
{
  ...
  if (m_80211mode == 3)
    {
      chAccessMode = 1;
      std::cout << "channel switching mode" << '\n';
    }
...
VanetRoutingExperiment::WriteCsvHeader ()
{
  ...
  out << "SimulationSecond," <<
    ...
    "ExpectedHPWavePktsReceived," <<
    "ExpectedWavePktsInCoverageReceived," <<
    "ExpectedHPWavePktsInCoverageReceived," <<
    "BSM_PDR1," <<
    "BSM_PDR2," <<
    "BSM_PDR3," <<
    "BSM_PDR4," <<
...
main (int argc, char *argv[])
{
  ...
  //std::cout << "file name" << m_CSVfileName3 <<'\n';
  //printStats(m_CSVfileName3);
}
